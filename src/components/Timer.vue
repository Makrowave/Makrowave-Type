...
const startTimer = () => {
    ...
    intervals.value.push(setInterval(() => { delta.value = Date.now() - startTime.value }, 10))
    intervals.value.push(setInterval(() => { emit('time', delta.value) }, 500))
}
const stopTimer = () => {
...
const time = computed(() => {
    ...
    const mil = Math.floor((time % 1000) / 10).toString().padStart(2, '0')
    return `${min}:${sec}:${mil}`
})
onUpdated(() => {
