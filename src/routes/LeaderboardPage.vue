...
<template>
  <div class="page-wrapper">
    ...
    <div class="box-shadow border-10 error" v-else>{{ dailyError }}</div>
    <AllTimeScoreboard v-if="allTimeError === ''" :scores="allTimeScores" />
    <div class="box-shadow border-10 error" v-else>{{ allTimeError }}</div>
  </div>
</template>
<style scoped>
.page-wrapper {
